...
\begin{enumerate}
    \item Založené (případně podobné) na jazyku MATLAB:
    \begin{itemize}
        \item MATLAB
        \item Simulink
        \item GNU Octave
        \item SciLab
    \end{itemize}
    \item Založené na jazyku Modelica:
    \begin{itemize}
        \item Dymola
        \item jModelica
        \item OpenModelica
...
Podobně jako u~MATLABu, i nástroj SciLab nabízí možnost využití vlastního webového rozhraní\footnote{https://www.scilab.org/cloud/web-application}. Přítomnost této možnosti používání bych označil za velkou výhodu oproti jiným volně dostupným nástrojům. Ani pro tento nástroj nebude tato možnost více rozebírána v~rámci této bakalářské práce.
...
Pro potřeby bakalářské práce je využit balík Assimulo\footnote{https://jmodelica.org/assimulo/}, pomocí kterého je možné řešit diferenciální rovnice. Tento balík funkcí je napsán právě v~jazyce Python.
...
\subsection{SciPy}
\label{chapter-scipy}
Tento simulační nástroj jako jediný není založený na jazycích MATLAB, či Modelica, nýbrž na skriptovacím jazyce Python. Není to simulační nástroj v~pravém slova smyslu, jedná se spíše o~rozšiřující knihovny jazyka Python, které jsou zdarma a volně dostupné\footnote{https://github.com/scipy/scipy} \cite{scipy}. V~rámci této rodiny knihoven jsou knihovny:
\begin{itemize}
...
\section{Shrnutí použitých jazyků a nástrojů}

V předchozích kapitolách jsme si představili zvolené simulační jazyky a následně nástroje, které jsou s těmito jazyky spjaty. Těmito jazyky jsou MATLAB, Modelica a Python (Scipy). V kapitole \ref{popis-nastroju} bylo představeno první možné rozdělení simulačních nástrojů. Dále ale také můžeme dělit simulační nástroje podle dostupnosti, a to takto:
\begin{enumerate}
    \item Komerční nástroje:
    \begin{itemize}
        \item MATLAB + Simulink
        \item Dymola
    \end{itemize}
    \item Volně dostupné nástroje:
    \begin{itemize}
        \item GNU Octave
        \item SciLab
        \item jModelica
...
Komerční nástroje jsou v této práci zahrnuty právě kvůli srovnání s volně dostupnými nástroji. Pro účely práce jsou využity licence zakoupené v rámci VUT FIT.

V kapitole \ref{kapitola5} lze nalézt následné porovnání simulačních nástrojů podle výsledků simulací s implementovanými modely popsanými v kapitole \ref{kapitola4}.
...
K vyhodnocování výsledků simulací je použita metodika k hodnocení nástrojů k simulování dopravy \cite{jones2004traffic}.  Původní převzatá metodika sledovala kritéria jako hardware a software náročnost, náročnost na kvalitu vstupních dat a pak další kritéria ve vztahu k použitým nástrojům. Pro účely bakalářské práce byla vybraná metodika modifikována podle potřeb zadání a následně byla vytvořena tato kritéria:
\begin{itemize}
    \item Strojový čas
    \item Přesnost zvolené numerické metody
    \item Hardwarové a softwarové požadavky
...
\label{implementace}
\todo{tahle kapitola az jako posledni v~pripade zmen implementaci}
...
V rámci této implementace je namodelován systém skákajícího míčku, který je z nedokonale pružného materiálu a postupně během skoků ztrácí svou rychlost a výšku odrazu. Míček je vypouštěn z výšky $h$, postupně nabere maximální rychlost $v_p$ a odrazí se od země rychlostí $v$, kterou lze popsat rovnicí:

$$
v = v_p \sqrt{1 - \exp(-2gh/v^2_p)}
$$
a následně vyletí do nové výšky $h$, která je definována rovnicí:

$$
h = -(v^2_p / g)\ln({\cos({\arctan({v_o / v_p})})})
$$
Parametr $v_o$ se vypočítá jako $v_o = C_r * v$, kde $C_r$ je zadaná hodnota parametru pružnosti. Pro účely experimentu jsou počáteční hodnoty nastaveny takto:
...
Pro implementaci posloužil jako základ kód pro jazyk MATLAB převzatý z oficiálního repozitáře společnosti MathWorks pro sdílení zdrojových kódů jazyka MATLAB\footnote{https://www.mathworks.com/matlabcentral/fileexchange/60525-experiments-with-a-bouncing-ball}. Výsledky implementovaného modelu a následné srovnání výsledků simulačních nástrojů lze nalézt v kapitole \ref{srovnani-micek}.
